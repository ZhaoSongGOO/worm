...
namespace worm {
Message MessageHeap::Pop() {
  AutoLock lock(lock_);
  if (min_heap_.empty()) {
    return Message();
  }
  Message node = min_heap_[0];
...
void MessageHeap::Remove(Message &node) {
  AutoLock lock(lock_);
  int index = -1;
...
void MessageHeap::Push(Message &node) {
  AutoLock lock(lock_);
  min_heap_.push_back(node);
  ShiftUp(min_heap_.size() - 1);
}

void MessageHeap::ShiftUp(int start) {
  int current = start;
...
void MessageHeap::ShiftDown(int start) {
  if (min_heap_.empty()) {
    return;
  }
